...
const deployedPath = path.resolve(
  "../blockchain_folder/address_data/deployedContract.json"
);
...
function loadContract(name, address) {
  const artifactPath = path.resolve(
    `../blockchain_folder/artifacts/smart_contracts/${name}.sol/${name}.json`
  );
...
const StorageManager = loadContract(
  "SCV_storage_manager",
  deployed.StorageManager
);
